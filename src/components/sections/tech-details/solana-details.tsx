  ...
  const ProgramCard = ({ program }: { program: { title: string; description: string; tags: string[] } }) => (
    ...
    >
      <div className="flex items-start justify-between mb-1.5">
        <h3 className="font-medium text-sm text-[#051A24]">{program.title}</h3>
      ...
      <p className="text-[#051A24]/65 text-sm mb-3 leading-relaxed">{program.description}</p>
      <a
        href={REPO_LINK}
        target="_blank"
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-4">
        <h1 className="mb-3 text-[32px] leading-[1.1] tracking-tight text-[#0D212C] md:text-[40px] lg:text-[44px]">
          Rust protocol <span className="font-serif">engineer.</span>
        </h1>
        <p className="text-[#051A24]/65 text-sm mb-4">Building secure, high-performance Solana smart contracts and programs</p>
        <a
          href={REPO_LINK}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 px-4 py-2 bg-[#051A24]/[0.05] hover:bg-[#051A24]/[0.08] text-[#051A24]/75 rounded-lg transition-colors border border-[#051A24]/10 text-sm"
        >
      ...
      <Card className="p-5">
        <h2 className="font-medium text-[#051A24]/50 text-xs uppercase tracking-wider mb-1">Anchor Programs</h2>
        <p className="text-[#051A24]/50 text-sm mb-4">Solana smart contracts built with Anchor framework</p>
      ...
      <Card className="p-5">
        <h2 className="font-medium text-[#051A24]/50 text-xs uppercase tracking-wider mb-1">Pinocchio Programs</h2>
        <p className="text-[#051A24]/50 text-sm mb-4">Lightweight no_std Solana programs using Pinocchio framework</p>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
      ...
      <Card className="p-5">
        <h2 className="font-medium text-[#051A24]/50 text-xs uppercase tracking-wider mb-1">TypeScript & Assembly</h2>
        <p className="text-[#051A24]/50 text-sm mb-4">Additional Solana programs in TypeScript and Assembly</p>
      ...
      <Card className="p-5">
        <h2 className="font-medium text-[#051A24]/50 text-xs uppercase tracking-wider mb-4">Certifications</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {certifications.map((cert, index) => (
            <div key={index} className="bg-white p-4 rounded-xl border border-[#051A24]/10">
              <h3 className="font-medium text-sm text-[#051A24] mb-1">{cert.name}</h3>
              <p className="text-[#051A24]/50 text-sm mb-2">by {cert.provider}</p>
              <a
                href={cert.link}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 text-[#051A24]/65 hover:text-[#051A24] text-sm transition-colors"
              >
